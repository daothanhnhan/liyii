...
class library{
	...
	/*------------- Hàm select list ----------------*/
	    ...
	    public function getList_hotnew($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	        ...
	        if (!empty($_SESSION['price'])) {
		        if ($_SESSION['price'] != '0-0') {
		            $price = explode("-", $_SESSION['price']);
		            $price1 = $price[0];
		            $price2 = $price[1];
		            if ($price1 < $price2) {
		                $where .= " AND product_price BETWEEN $price1 and $price2";
		            } else {
		                $where .= " AND product_price >= $price1";
		            }
		        }
		        $cond .= "&price=".$_SESSION['price'];
		    }

		    if (!empty($_SESSION['size'])) {
		        $size = $_SESSION['size'];
		        $where .= " AND kich_co_id like '%\"$size\"%'";
		        $cond .= "&size=$size";
		    }
		    ...
		    if (!empty($_SESSION['color'])) {
		        ...
		        foreach ($_SESSION['color'] as $item) {
		            $where .= " AND mau_sac_id like '%\"$item\"%'";
		            $cond .= "&color[]=$item";
		        }
		        $where .= ")";
		    }
	        ...
	        if ($trang != "" && $limit != "") {
	            ...
	            $config = array(
	                ...
	                'link_full'=> $slug == '' ? '/'.$page.'/{trang}' : 'index.php?page='.$page.$cond.'&trang={trang}',

	                'link_first'=> $slug == '' ? '/'.$page : 'index.php?page='.$page.$cond,
